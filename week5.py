...
def joint_probability(alarm, burglary, earthquake, david_calls, sophia_calls):#(t,f,f,t,t)
    if alarm:
        if burglary and earthquake:
            P_alarm = P_alarm_given_burglary_and_earthquake #0.94
        elif burglary:
            P_alarm = P_alarm_given_burglary_and_no_earthquake #0.95
        elif earthquake:
            P_alarm = P_alarm_given_no_burglary_and_earthquake #0.31
        else:
            P_alarm = P_alarm_given_no_burglary_and_no_earthquake#0.001
    else:
        if burglary and earthquake:
            P_alarm = 1 - P_alarm_given_burglary_and_earthquake #0.06
        elif burglary:
            P_alarm = 1 - P_alarm_given_burglary_and_no_earthquake #0.05
        elif earthquake:
            P_alarm = 1 - P_alarm_given_no_burglary_and_earthquake #0.69
        else:
            P_alarm = 1 - P_alarm_given_no_burglary_and_no_earthquake #0.999
    ...
    if alarm:
        if david_calls:
            ...
            P_david=P_david_does_not_calls_given_alarm
    else:
        if david_calls:
            P_david=P_david_calls_given_no_alarm
        else:
            P_david=P_david_does_not_calls_given_no_alarm
